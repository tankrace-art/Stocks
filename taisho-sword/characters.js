...
export const CHARACTERS = [
  {
    ...
    colors: { kimono: 0x2d4370, hakama: 0x1b1f33, haori: 0x1c1c24, scarf: 0xf1e9d6, hair: 0x15111a, obi: 0x8e2b2b, glow: 0x40a0ff },
    look: { cap: true },
    stats: { hp: 120, speed: 6.2, dmg: 1.0, gauge: 1.0, dashCooldown: 0.75 },
    special: { name: '비검·파염', theme: 'wave', c1: 0x6fb6ff, c2: 0xff8a2a, c3: 0xffffff, range: 4.6, dmg: 55, waves: 1 },
    bars: { atk: 3, spd: 3, hp: 3, tec: 3 },
  },
  {
    id: 'rai', name: '라이', style: '뇌명류(雷鳴流)', role: '속도형',
    desc: '번개처럼 파고들어 베고 빠진다. 대시가 빠르고 체력은 낮다.',
    colors: { kimono: 0xe8c23a, hakama: 0x1a1a1a, haori: 0x2a2a2a, scarf: 0xffffff, hair: 0xf0e6c8, obi: 0x222222, glow: 0xfff070 },
    look: { cap: false, spikyHair: true },
    stats: { hp: 100, speed: 7.3, dmg: 0.9, gauge: 1.1, dashCooldown: 0.45 },
    special: { name: '비검·뇌명', theme: 'lightning', c1: 0xfff27a, c2: 0xffffff, c3: 0x9ad8ff, range: 4.2, dmg: 48, waves: 1, waveSpeed: 24 },
    bars: { atk: 2, spd: 5, hp: 2, tec: 3 },
  },
  {
    id: 'kuma', name: '쿠마', style: '맹수류(猛獸流)', role: '중량형',
    desc: '곰 같은 힘으로 대검을 휘두른다. 느리지만 한 방이 무겁고 튼튼하다.',
    colors: { kimono: 0x5a3a26, hakama: 0x2a1e18, haori: 0x7a2a22, scarf: 0xd9a640, hair: 0x2a1a12, obi: 0x1a1a1a, glow: 0xff9040 },
    look: { cap: false, headband: true, bladeScale: 1.45 },
    stats: { hp: 155, speed: 5.3, dmg: 1.35, gauge: 0.85, dashCooldown: 0.9 },
    special: { name: '비검·지명', theme: 'quake', c1: 0xffb060, c2: 0x8a6a40, c3: 0xff5030, range: 5.2, dmg: 70, waves: 1, waveSpeed: 11 },
    bars: { atk: 5, spd: 1, hp: 5, tec: 2 },
  },
  {
    id: 'hino', name: '히노', style: '홍염류(紅炎流)', role: '공격형',
    desc: '타오르는 검기로 요괴를 태운다. 공격력이 높고 공세가 거칠다.',
    colors: { kimono: 0xb8322a, hakama: 0x2a1212, haori: 0x3a1a1a, scarf: 0xffd9a0, hair: 0x3a1010, obi: 0xd9a640, glow: 0xff5020 },
    look: { cap: true },
    stats: { hp: 110, speed: 6.0, dmg: 1.18, gauge: 1.0, dashCooldown: 0.75 },
    special: { name: '비검·홍련', theme: 'flame', c1: 0xff6a20, c2: 0xffd050, c3: 0xff2a10, range: 4.8, dmg: 62, waves: 1 },
    bars: { atk: 4, spd: 3, hp: 2, tec: 3 },
  },
  {
    id: 'kaze', name: '카제', style: '청풍류(靑風流)', role: '기술형',
    desc: '바람을 타는 삿갓의 검사. 기술 게이지가 빨리 차고 비검의 범위가 넓다.',
    colors: { kimono: 0x2f7a5a, hakama: 0x1a2a24, haori: 0xe8e0cc, scarf: 0x9ad8c0, hair: 0x1a1a1a, obi: 0x3a3a3a, glow: 0x80ffc0 },
    look: { cap: false, kasa: true },
    stats: { hp: 105, speed: 6.6, dmg: 0.95, gauge: 1.45, dashCooldown: 0.65 },
    special: { name: '비검·선풍', theme: 'wind', c1: 0x9affc8, c2: 0xffffff, c3: 0x60d090, range: 5.6, dmg: 50, waves: 2 },
...
export class Progress {
  constructor(charId) {
    ...
    this.items = []; this.equipped = { weapon: null, armor: null, charm: null }; this.potions = 2;
    this.load();
  }
    ...
    try {
      ...
      if (raw) {
        ...
        Object.assign(this, {
          ...
          items: d.items || [], equipped: Object.assign({ weapon: null, armor: null, charm: null }, d.equipped || {}),
          potions: d.potions === undefined ? 2 : d.potions,
        });
      }
    } catch (_) { /* 저장소 없음 */ }
  }
  save() {
    try {
      localStorage.setItem(this.key, JSON.stringify({
        ...
        items: this.items, equipped: this.equipped, potions: this.potions,
      }));
    } catch (_) { /* ignore */ }
  }
  // ---- 아이템 ----
