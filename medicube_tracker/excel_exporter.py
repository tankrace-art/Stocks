...
def _create_amazon_rank_history_sheet(
    ...
    """
    Single unified Amazon rank history sheet — product × (date × country) matrix.
    Layout (per brand):
      Row 1: [Brand title — full width]
      Row 2: [상품명 (spans rows 2-3)] | [date1 merged over N country cols] | [date2 ...] | ...
      Row 3:                             | [C1] | [C2] | ... | [C1] | [C2] | ...
      Row 4+: product A                 | rank | rank | ... | rank | rank | ...
    """
    from .rank_history import get_sorted_dates
    ...
    countries = [ak.replace("amazon_", "") for ak in amazon_keys]

    # Collect all dates across all countries
    ...
    # col_pairs: ordered list of (date, country) for each data column
    # col 1 = product name; col 2+ = data columns in (date, country) order
    col_pairs = [(d, c) for d in dates for c in countries]
    total_data_cols = len(col_pairs)
    total_cols = max(2, 1 + total_data_cols)
    ...
    for brand_key, brand_label, brand_color in [
        ...
        # Collect all products for this brand across all countries
        all_prods: set[str] = set()
        for ak in amazon_keys:
            all_prods.update(history.get(ak, {}).get(brand_key, {}).keys())
        if not all_prods:
            continue

        # Sort products: best average rank first; unranked products last
        def sort_key(prod_name):
            ranks = [
                rv
                for ak in amazon_keys
                for rv in history.get(ak, {}).get(brand_key, {}).get(prod_name, {}).values()
                if rv is not None
            ]
            return (len(ranks) == 0, sum(ranks) / max(1, len(ranks)))

        sorted_prods = sorted(all_prods, key=sort_key)

        # ── Row: Brand title ──────────────────────────────────────────
        title_cell = ws.cell(row=current_row, column=1)
        title_cell.value = f"Amazon 베스트셀러 순위이력  |  {brand_label}"
        title_cell.font = Font(bold=True, size=13, color="FFFFFF", name="맑은 고딕")
        title_cell.fill = PatternFill("solid", fgColor=brand_color)
        title_cell.alignment = Alignment(horizontal="center", vertical="center")
        ...
        country_header_row = current_row + 1

        # ── Row: "상품명" header (spans date+country rows) ────────────
        nc = ws.cell(row=date_header_row, column=1)
        nc.value = "상품명"
        nc.font = Font(bold=True, size=10, color="FFFFFF", name="맑은 고딕")
        nc.fill = PatternFill("solid", fgColor="4472C4")
        nc.alignment = Alignment(horizontal="center", vertical="center")
        nc.border = _thin_border()
        ws.merge_cells(
            start_row=date_header_row, start_column=1,
            end_row=country_header_row, end_column=1,
        )

        # ── Row: Date headers (each date merges over its country sub-cols) ──
        for d_idx, d in enumerate(dates):
            col_start = 2 + d_idx * len(countries)
            col_end = col_start + len(countries) - 1
            c = ws.cell(row=date_header_row, column=col_start)
            c.value = fmt_date(d)
            ...
            if col_start < col_end:
                ws.merge_cells(
                    start_row=date_header_row, start_column=col_start,
                    end_row=date_header_row, end_column=col_end,
                )
        ws.row_dimensions[date_header_row].height = 20

        # ── Row: Country headers ───────────────────────────────────────
        for col_idx, (_, country) in enumerate(col_pairs, 2):
            cell = ws.cell(row=country_header_row, column=col_idx)
            cell.value = country
            cell.font = Font(bold=True, size=9, color="FFFFFF", name="맑은 고딕")
            cell.fill = PatternFill("solid", fgColor=COLOR_SUBHEADER_BG)
            cell.alignment = Alignment(horizontal="center", vertical="center")
            cell.border = _thin_border()
            col_letter = get_column_letter(col_idx)
            ws.column_dimensions[col_letter].width = max(
                ws.column_dimensions[col_letter].width or 0, 8
            )
        ws.row_dimensions[country_header_row].height = 18
        current_row += 2

        # ── Product rows ───────────────────────────────────────────────
        for i, prod_name in enumerate(sorted_prods):
            bg = COLOR_ROW_ALT if i % 2 == 0 else None
            ...
            for col_idx, (d, country) in enumerate(col_pairs, 2):
                ak = f"amazon_{country}"
                rv = history.get(ak, {}).get(brand_key, {}).get(prod_name, {}).get(d)
                cell = ws.cell(row=current_row, column=col_idx)
                cell.value = rv if rv is not None else ""
                rank_bg = _rank_cell_color(rv)
                cell.font = Font(size=9, bold=(rv is not None and rv <= 10), name="맑은 고딕")
                if rank_bg:
                    cell.fill = PatternFill("solid", fgColor=rank_bg)
                elif bg:
                    cell.fill = PatternFill("solid", fgColor=bg)
